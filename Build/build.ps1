...
'@
    ...
    # Determine if there are any modules that require bootstrapping
    if (Get-Content $Script | Select-String -Pattern '^\s*#module')
    {
        Write-Host "Found #module in $(Split-Path -Leaf $Script) - Bootstrapping..."
        Invoke-Expression "& $DotNetExpression `"$CAKE`" `"$Script`" --bootstrap -verbosity=`"$Verbosity`""
    }
